...
左の「使われ方」に、どこがよく押されているかが出る。

**誰が押したかは記録していない。** 残すのは「日・種類・場所・回数」の4つ
だけで、利用者を特定する列そのものが無い (`usage` シート)。個人の操作を
追える形にすると、見られている道具になり、率直に使われなくなるためである。
知りたいのは「どこが使われていないか」であって「誰が何をしたか」ではない。

そのため、このボードは誰が見てもかまわない。
